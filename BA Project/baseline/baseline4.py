# see how the grid_distance/speed does
import numpy as np 
import pandas as pd 

order = pd.read_csv('order_20200201.csv')
action = pd.read_csv('action_20200201.csv')
courier = pd.read_csv('courier_20200201.csv')
distance = pd.read_csv('distance_20200201.csv')
...
for i in range(len(action_data)):
    ...
    if i == 0:
        # print('#####', order.loc[order['tracking_id']==tracking_id])
        # start_time = action.loc[action['tracking_id']==tracking_id, 'expect_time'].item()
        e = action.loc[action['tracking_id']==tracking_id]
        start_time = e.loc[e['action_type']=='PICKUP', 'expect_time'].item()
...
out.to_csv('baseline4_result.csv', index = False)